...
#define DDOT_MAX_SAFE_TILE 100000000
...
int main(const int argc, const char *argv[]) {
	...
	if (predef_control_values!= NULL){
		...
		sprintf(filename, "%s/CoCoPeLiaDdotRunnerBest_predefined_vals_%s_%s_%s.log",
			TESTLIBDIR, CoCoDistributionPrint(), CoCoImplementationPrint(), VERSION);
	}
	else sprintf(filename, "%s/CoCoPeLiaDdotRunnerBest_%s_%s_%s.log",
		TESTLIBDIR, CoCoDistributionPrint(), CoCoImplementationPrint(), VERSION);
#ifdef CHECKLOG
	CheckLogLvl1(filename, predef_control_values, 0, N, incx, incy, x_loc, y_loc, x_out_loc, y_out_loc);
#endif
	...
	else return_values = PARALiADdot(N, x, incx, y, incy, result);
	CoCoSyncCheckErr();
	...
	//StoreLogLvl1(filename, predef_control_values, N, incx, incy, x_loc, y_loc, x_out_loc, y_out_loc, cpu_timer);
  ...
  short bench_it = 100;
	...
	avg_t/=bench_it;
	fprintf(stderr, "CoCopeLia Ddot(%s):\n\tfirst_it_t = %lf ms ( %lf Gflops/s )\n\tavg_t = %lf ms ( %lf Gflops/s )\n\tmin_t = %lf ms ( %lf Gflops/s )\n\tmax_t = %lf ms ( %lf Gflops/s )\n",
	return_values->print_csv(),
	first_over_t  * 1000, Gval_per_s(dot_flops(N),first_over_t),
	avg_t  * 1000, Gval_per_s(dot_flops(N),avg_t),
