...
CoControl_p PARALiaDgemm(char TransA,  char TransB, size_t M, size_t N, size_t K, double alpha, double* A, size_t ldA, double* B, size_t ldB, double beta, double* C, size_t ldC);

/// A modification of PARALiaDgemm but with a given T (mainly for performance/debug purposes)
CoControl_p PARALiaDgemmControled(char TransA,  char TransB, size_t M, size_t N, size_t K, double alpha, double* A, size_t ldA, double* B, size_t ldB, double beta, double* C, size_t ldC, CoControl_p predef_control_values);
...
///Clears all GPU mem allocation done by PARALiaDgemm at target device
void PARALiaDgemm_flush_gpu_mem_buf(short dev_id);
#endif
