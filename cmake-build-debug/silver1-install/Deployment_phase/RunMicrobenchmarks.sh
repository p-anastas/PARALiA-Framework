...
BIN_DIR=/tmp/tmp.YXAEzsRmOZ/cmake-build-debug/silver1-install/Deployment_phase/bin
...
micro_dgemm_gpu_exec="${BIN_DIR}/RunCuCuBlasDgemmBench"
micro_dgemm_cpu_exec="${BIN_DIR}/RunCBlasDgemmBench"
...
do
	for TransB in N T;
	do
		echo "$micro_dgemm_gpu_exec $device $TransA $TransB &>> $dgemm_log"
		$micro_dgemm_gpu_exec $device $TransA $TransB &>> $dgemm_log
		echo "$micro_dgemm_cpu_exec $TransA $TransB &>> $dgemm_log"
		$micro_dgemm_cpu_exec $TransA $TransB &>> $dgemm_log
		echo "Done"
	done 
done
