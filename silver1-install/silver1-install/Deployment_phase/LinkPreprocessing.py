...
for other_dev_id in range(-1,devices-1):
	...
	for index, row in indata_recv_over.iterrows():
		#print(row)
		#print('Slowdown([%d]->[%d]): ' % (row['over_loc_dest'], row['over_loc_src']) + str(row['over_transfer_t']/row['transfer_t']))
		f.write('Slowdown([%d]->[%d]): ' % (row['over_loc_dest'], row['over_loc_src']) + str(row['over_transfer_t']/row['transfer_t']) + '\n')
	f.close()
