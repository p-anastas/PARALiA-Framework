...
int main(const int argc, const char *argv[]) {
    ...
    A_buf = (double*) CoCoMalloc(maxDim * maxDim * sizeof(double), dev_id, 0);
  	x_buf = (double*) CoCoMalloc(maxDim * sizeof(double), dev_id, 0);
  	y_buf = (double*) CoCoMalloc(maxDim * sizeof(double), dev_id, 1);
	CoCoSyncCheckErr();
	...
	for ( int itt = 0; itt <1; itt++){
		if(dev_id != -1){
			assert(CUBLAS_STATUS_SUCCESS == cublasDgemv(handle0, gpu_op_A,
	...
	for (long int T = MIN_DIM_BLAS2; T < MAX_DIM_BLAS2; T+=step){
		if (T >= step * 16) step *=2;
		fprintf(stderr,"Running cublasDgemv-> square T = %d:\n", T);
