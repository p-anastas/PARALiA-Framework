...
int main(const int argc, const char *argv[]) {
	...
	if (dev_id != -1){
		...
		massert(cudaSuccess == cudaMemGetInfo(&free_cuda_mem, &max_cuda_mem),
			"sgemm_microbench_gpu: cudaMemGetInfo failed");
	...
	float *A_buf, *B_buf, *C_buf;
  	A_buf = (float*) CoCoMalloc(maxDim * maxDim * sizeof(float), dev_id, 0);
  	B_buf = (float*) CoCoMalloc(maxDim * maxDim * sizeof(float), dev_id, 0);
  	C_buf = (float*) CoCoMalloc(maxDim * maxDim * sizeof(float), dev_id, 1);
	CoCoSyncCheckErr();
	...
	for ( int itt = 0; itt <1; itt++){
		if(dev_id != -1){
			assert(CUBLAS_STATUS_SUCCESS == cublasSgemm(handle0, gpu_op_A, gpu_op_B,
