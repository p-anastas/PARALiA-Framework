	...
	double* active_unit_score)
{
	switch(model->problem){
		case BLAS1:
			error("PredictHeteroFullOverlap_v2: BLAS 1 Not implemented\n");
		case BLAS2:
			error("PredictHeteroFullOverlap_v2: BLAS 2 Not implemented\n");
		case BLAS3:
			return PredictHeteroFullOverlapBLAS3_v2(model, T, active_unit_num, active_unit_id_list,
				active_unit_score);
		default:
			error("PredictHeteroFullOverlap_v2: Invalid Problem %s", printProblem(model->problem));
	}
	return 0;
}
