		...
		if (TileDimlist[j] == 1)
			error("Subkernel(dev=%d,id=%d)::request_data: Tile1D not implemented\n", run_dev_id, id);
				...
				if (tmp->CacheLocId[run_dev_id] < -1){
					...
					if (tmp->W_flag && WR_reader){
						//delete tmp->available[InitialId]; // FIXME is this dangerous?
						tmp->available[InitialId] = new Event();
					}
					if (tmp->W_flag) CoCacheAddPendingEvent(run_dev_id, tmp->available[run_dev_id], tmp->available[InitialId], tmp->CacheLocId[run_dev_id], W);
					...
					if (tmp->W_flag) CoCacheAddPendingEvent(run_dev_id, tmp->available[run_dev_id], tmp->available[InitialId], tmp->CacheLocId[run_dev_id], W);
					else if(tmp->R_flag) CoCacheAddPendingEvent(run_dev_id, tmp->available[run_dev_id], operation_complete, tmp->CacheLocId[run_dev_id], R);
				}
		}
