...
int main(const int argc, const char *argv[]) {
	...
	if (predef_control_values!= NULL){ 
		if(predef_control_values->T > 0) {
			...
			sprintf(filename, "%s/BLASxExDgemmRunner_predefined_vals.log", TESTLIBDIR);	
		}
	}
	else sprintf(filename, "%s/BLASxExDgemmRunner.log", TESTLIBDIR);

	size_t BLASx_tile;
	if (predef_control_values!= NULL && predef_control_values->T > 0) return_values->T = BLASx_tile = predef_control_values->T;
	else return_values->T = BLASx_tile = 4096; // The default tile for our machine
	double cpu_ratio;
